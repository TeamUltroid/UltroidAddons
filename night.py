# Credits sensible userbot (creator of plugin: sensible userbot and Ported by @TheShashank😎😎)   

   

# Ported from (SENSIBLE USERBOT)   
    ...
    if ".night" not in current_message and "yes" in USER_night:  # pylint:disable=E0602
        try:
            await borg.send_message(  # pylint:disable=E0602
                ...
                "Mine Owner has Gone Sleep (Pure Din Sota hi Rehta He AALSII MASTER😶 {DEFAULTUSER} )",
            )
...
async def on_night(event):
    ...
    if USER_night and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        message_to_reply = (
            ...
            if reason
            else f"**Important Notice**\n\n[{DEFAULTUSER} MERE PYAARE DULARE MASTER JII🥵🥵 SO RHE HAI..KRIPYA UNHE DE🤗🤗](https://telegra.ph/file/6a6f5db6d218a4474940f.jpg) "
        )
