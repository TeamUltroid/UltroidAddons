# credits @xditya & @Maxprogrammer007  for editing
...
@ultroid.on(admin_cmd(pattern="sg ?(.*)"))
@ultroid.on(sudo_cmd(pattern="sg ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
 ...
 (http://reply_message.sender.id/)    if reply_message.sender.bot (http://reply_message.sender.bot/):
    ...
    async with event.client.conversation(chat) as conv:
        ...
        if response1.text.startswith("No records found"):
            ...
            await event.delete()
            await ultroid.send_message(event.chat_id, response2.message)

            await ultroid.send_message(event.chat_id, response3.message)
